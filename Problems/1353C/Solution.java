...
public class Solution {
	public static void main(String args[]) throws IOException {
		...
		int t = r_i(br);
		while (t > 0) {
			BigInteger n = new BigInteger(r_i(br) + "");

			BigInteger p = (n.subtract(BigInteger.ONE)).divide(new BigInteger(2 + ""));

			BigInteger e_p = (p.multiply(p.add(BigInteger.ONE))).divide(BigInteger.TWO);
			BigInteger ans = e_p.multiply(new BigInteger(8 + ""));

			pr_o_nl(ans.add(rest(p)));
	...
	public static BigInteger rest(BigInteger p) {
		BigInteger ans = BigInteger.ZERO;
		BigInteger init = new BigInteger("2");
		for (int i = 0; i < p.intValue() - 1; i++) {
			ans = ans.add(init.multiply(new BigInteger((i + 1) + "")));
			init = init.add(BigInteger.ONE);
		}
		return ans.multiply(new BigInteger(8 + ""));
	}
