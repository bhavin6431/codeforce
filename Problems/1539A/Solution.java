	...
	public static void main(String args[]) throws IOException {
		...
		while (tc > 0) {
			...
			if (x > t) {
				...
				BigInteger o_l = new BigInteger((int) Math.floor(t / x) + "");
				o_l = new BigInteger((int) Math.min(o_l.intValue(), n - 1) + "");
				BigInteger ans = (o_l.multiply(o_l.add(BigInteger.ONE))).divide(BigInteger.TWO);
				int v_m = Math.max(0, (n - 1) - o_l.intValue());

				ans = ans.add(new BigInteger(v_m + "").multiply(o_l));
				pr_o_nl(ans);
			}
			tc--;
		}
