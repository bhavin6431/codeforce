...
public class Solution {
	public static void main(String args[]) throws IOException {
		...
		while (t > 0) {
			...
			// for (int n = 1; n <= 100; n++) {
			int s = 1;
			int mid = (int) Math.ceil((double) n * n / 2) + 1;
			int m[][] = new int[n][n];
			boolean f = true;
			boolean w = true;
			int r = 0;
			...
			for (int i = 0; i < n * n; i++) {
				m[r][j] = f ? s++ : mid++;
				if ((r - 1 >= 0 && m[r - 1][j] != 0 && Math.abs(m[r - 1][j] - m[r][j]) < 2)
						|| (j - 1 >= 0 && m[r][j - 1] != 0 && Math.abs(m[r][j - 1] - m[r][j]) < 2)
						|| (r + 1 < n && m[r + 1][j] != 0 && Math.abs(m[r + 1][j] - m[r][j]) < 2)
						|| (j + 1 < n && m[r][j + 1] != 0 && Math.abs(m[r][j + 1] - m[r][j]) < 2)) {
					w = false;
					break;
				}
