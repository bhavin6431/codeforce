...
ll t,x,y, n,maxn, k, m, tc,d,l , r, a, b, ans, c, i,j,q ,val,z, sum;
string s1, s2;
...
int main() {
    ...
    while(t--){
        ...
        loop(i, 0, n) {
            read(val);  
            ll changeR = val - 25;
            if(changeR > 0 && change < changeR) {
                can = !can;
                break;
            }
            int v1 =  min(store[0], changeR / 100);
            store[0]-= v1;
            changeR -= v1 * 100;
            v1 = min(store[1], changeR / 50);
            store[1] -= v1;
            changeR -= v1 * 50;
            v1  = min(store[2], changeR / 25);
            store[2] -= v1; 
            changeR -= v1 * 25;
            if(changeR > 0) {
                can = !can;
