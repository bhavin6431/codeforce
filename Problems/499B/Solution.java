...
public class Solution {
	public static void main(String args[]) throws IOException {
		...
		int n = in[0];
		int m = in[1];
		Map<String, String> ott = new HashMap<>();
		Map<String, String> tto = new HashMap<>();
		for (int i = 0; i < m; i++) {
			String[] s = r_s_a(br);
			ott.put(s[0], s[1]);
			tto.put(s[1], s[0]);
		...
		String[] s = r_s_a(br);
		for (int i = 0; i < n; i++) {
			if (ott.containsKey(s[i])) {
				if (s[i].length() == ott.get(s[i]).length() || s[i].length() < ott.get(s[i]).length()) {
					pr_o(s[i] + " ");
				} else {
					pr_o(ott.get(s[i]) + " ");
				}
			} else {
				if (tto.get(s[i]).length() == s[i].length() || tto.get(s[i]).length() < s[i].length()) {
					pr_o(tto.get(s[i]) + " ");
				} else {
					pr_o(s[i] + " ");
				}
