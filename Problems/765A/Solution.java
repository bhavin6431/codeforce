...
public class Solution {
	public static void main(String args[]) throws IOException {
		...
		int n = r_i(br);
		String startOrig = r_s(br);
		String trip[] = new String[n];
		Map<String, Map<String, Integer>> map = new HashMap<>();
		for (int i = 0; i < n; i++) {
			trip[i] = r_s(br);
			String s1[] = trip[i].split("->");
			if (map.containsKey(s1[0])) {
				if (map.get(s1[0]).containsKey((s1[1]))) {
					map.get(s1[0]).put(s1[1], map.get(s1[0]).get(s1[1]) + 1);
				} else {
					map.get(s1[0]).put(s1[1], 1);
				}

			} else {
				Map<String, Integer> newm = new HashMap<>();
				newm.put(s1[1], 1);
				map.put(s1[0], newm);
			}
		}

		String start = startOrig;
		while (map.get(start) != null && map.get(start).size() > 0) {
			int val = map.get(start).values().iterator().next();
			String newstart = map.get(start).keySet().iterator().next();
			if (val == 1) {
				map.get(start).remove(map.get(start).keySet().iterator().next());
			} else {
				map.get(start).put(map.get(start).keySet().iterator().next(), val - 1);
			}
			start = newstart;
		}
		if (start.equals(startOrig)) {
			pr_o_nl("home");
		} else {
			pr_o_nl("contest");
		}
