...
public class Solution {
	...
	public static void main(String args[]) throws IOException {
		...
		while (t > 0) {
			...
			int m = Math.min(x, y);
			int costofTwo = Math.min(a * 2, b);
			int diff = Math.abs(x - y);
			BigInteger ans = new BigInteger(m + "").multiply(new BigInteger(costofTwo + ""));
			ans = ans.add((new BigInteger(diff + "").multiply(new BigInteger(a + ""))));
			pr_o_nl(ans.toString());
			t--;
		}
	}
