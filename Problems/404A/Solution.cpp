...
int main() {
    ...
    char mc = c[(n - 1)/2][(n - 1)/2];
    if(s.size() != 2 || s[mc] != (2 * n) - 1 ) {
        p_e("NO");
    } else {
        bool valid = true;
        
        int i1 = (n - 1)/2, j1 = (n - 1)/2, i2 = (n - 1)/2, j2= (n - 1)/2, i3= (n - 1)/2, j3 = (n - 1)/2, i4  = (n - 1)/2, j4 = (n - 1)/2;
        while(i1 >=0) {
            if(c[i1][j1] == mc && c[i2][j2] == mc && c[i3][j3] == mc && c[i4][j4] == mc) {
                i1--;
                j1--;
