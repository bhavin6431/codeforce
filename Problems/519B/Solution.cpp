...
int main() {
    ...
    vector<int> c2;
    read_vector_i(e, n);
    read_vector_i(c1, n - 1);
    read_vector_i(c2, n - 2);
    pute(accumulate(e.begin(), e.end(), 0) - accumulate(c1.begin(), c1.end(), 0));
    pute(accumulate(c1.begin(), c1.end(), 0)- accumulate(c2.begin(), c2.end(), 0));
