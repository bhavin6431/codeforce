...
public class Solution {
	...
	public static void main(String args[]) throws IOException {
		...
		boolean cdu = u < d ? true : false;
		boolean cud = u > d ? true : false;
		int diff = Math.min(Math.abs((n / 2) - u), Math.abs((n / 2) - d));
		pr_o_nl(u != d ? diff : "0");
		if (diff != 0) {
			for (int i = 0; i < n; i++) {
				if (cdu && diff > 0 && s.charAt(i) == 'x') {
					pr_o("X");
					diff--;
				} else if (cud && diff > 0 && s.charAt(i) == 'X') {
					pr_o("x");
					diff--;
				} else {
					pr_o(s.charAt(i));
