...
public class Solution {
					...
					for (int k = s + 2; k < n; k++) {
						...
						for (int i = s; i <= k - 2; i++) {
							for (int j = i + 1; j <= k - 1; j++) {
								isGood = isGood(in[i], i, in[j], j, in[k], k);
								if (!isGood) {
									break;
								}
							}
	...
	static boolean isGood(int bi, int i, int bj, int j, int bk, int k) {
		// pr_o_nl("Checking : " + bi + "-" + bj + "-" + bk + "-");
		// pr_o_nl("for : " + i + "-" + j + "-" + k + "-");
		int dij = Math.abs(bi - bj) + Math.abs(i - j);
		int djk = Math.abs(bj - bk) + Math.abs(j - k);
		int dik = Math.abs(bi - bk) + Math.abs(i - k);

		return dik != dij + djk && dij != dik + djk && djk != dik + dij;
	}
