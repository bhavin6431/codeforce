...
bool canInvite(vector<pii> &criteria, int k, int n) {
    int i = 0;
    c = 0;
    while(k && i < n) {
        if(criteria[i].first >= k - 1 && criteria[i].second >= c) {
            k--;
            c++;
        }
        i++;
...
void solve() {
    read(n);
    vector<pii> criteria(n);
    loop(i, 0, n) {
        read2(criteria[i].first, criteria[i].second);
    }
