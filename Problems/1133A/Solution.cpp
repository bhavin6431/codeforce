...
int main() {
    ...
    read(hm);
    read(h1m1);
    vector<string> hmt = tokenize(hm, ':');
    vector<string> hm1t = tokenize(h1m1, ':');
    
    int mins = stoi(hmt[0]) * 60 + stoi(hmt[1]);
    int mine = stoi(hm1t[0]) * 60 + stoi(hm1t[1]);
    int diff = (mine - mins) / 2;
    debug(diff);
    int h = diff / 60;
    debug(h);
    diff = diff - (h * 60); 
    mins = stoi(hmt[1]) + diff;
    debug(mins);
    if(mins >= 60) {
        mins = mins - 60;
        hmt[0] = to_string(stoi(hmt[0]) + 1);
    }
    hmt[0] = to_string(stoi(hmt[0]) + h);

    string ha = mins < 10 ? "0" + to_string(mins) : to_string(mins);
    put(stoi(hmt[0]) < 10 ? "0" + hmt[0] : hmt[0], ":", ha);
